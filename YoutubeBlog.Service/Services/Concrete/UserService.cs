...
namespace YoutubeBlog.Service.Services.Concrete
{
    public class UserService : IUserService
    {
        ...
        public async Task<(IdentityResult identityResult, string? email)> Delete(Guid userId)
        {
        ...
        public async Task<bool> UserProfileUpdateAsync(UserProfileDto userProfileDto)
        {
            ...
            var isVerified = await userManager.CheckPasswordAsync(user, userProfileDto.CurentPassword);
            if (isVerified && userProfileDto.NewPassword != null)
            {
                var result = await userManager.ChangePasswordAsync(user, userProfileDto.CurentPassword, userProfileDto.NewPassword);
                if (result.Succeeded)
                {
